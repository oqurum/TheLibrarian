...
impl BookPersonModel {
	...
	pub fn remove_by_book_id(id: usize, db: &Database) -> Result<()> {
		db.lock()?
	...
	pub fn remove_by_person_id(id: usize, db: &Database) -> Result<()> {
		db.lock()?
	...
	pub fn transfer(from_id: usize, to_id: usize, db: &Database) -> Result<usize> {
		Ok(db.lock()?
