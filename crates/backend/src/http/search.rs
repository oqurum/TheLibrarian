...
pub async fn public_search_book(
    ...
    let GetSearchQuery { query, offset, limit, view_private, server_id } = query.into_inner();
    ...
    if query.starts_with(ID_CHECK) && query.len() > 3 {
        let book_id = BookId::from(query[ID_CHECK.len()..].parse::<usize>().map_err(Error::from)?);
        ...
        if offset == 0 {
            ...
            if search_item_updated {
                ...
                NewSearchGroupModel::new(query.clone(), total).insert_or_inc(&db).await?;
            }
        }
...
pub async fn public_search_author(
    ...
    if query.query.starts_with(ID_CHECK) && query.query.len() > 3 {
        ...
        let total = PersonModel::search_count(
            &query.query,
            &db,
        ).await?;
        ...
        Ok(web::Json(WrappingResponse::okay(PublicSearchType::AuthorList(QueryListResponse {
            offset,
            limit,
            total,
            items: items.into_iter()
